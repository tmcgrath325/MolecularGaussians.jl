...
function gogma_tform(X)
    return AffineMap(GaussianMixtureAlignment.rotmat(X[1],X[2],X[3]), SVector(X[4],X[5],X[6]))
end
...
function gogma_align_gmms(gmm1::Union{MolGMM,FeatureMolGMM}, gmm2::Union{MolGMM,FeatureMolGMM}, nsplits=2; atol=0.1, rtol=0.01, maxblocks=5e8, maxstagnant=Inf, maxevals=Inf)
    res = GaussianMixtureAlignment.gogma_align(gmm2.model, gmm1.model, nsplits, atol=atol, rtol=rtol, maxblocks=maxblocks, maxevals=maxevals, maxstagnant=maxstagnant, threads=threads)
...
function rot_gogma_align_gmms(gmm1::Union{MolGMM,FeatureMolGMM}, gmm2::Union{MolGMM,FeatureMolGMM}, nsplits=2, trl=nothing; atol=0.1, rtol=0.01, maxblocks=5e8, maxstagnant=Inf, maxevals=Inf)
    res = GaussianMixtureAlignment.rot_gogma_align(gmm2.model, gmm1.model, nsplits, trl, atol=atol, rtol=rtol, maxblocks=maxblocks, maxevals=maxevals, maxstagnant=maxstagnant)
    rx, ry, rz = res[3]
    R = GaussianMixtureAlignment.rotmat(rx, ry, rz)
    if isnothing(trl)
...
function tiv_gogma_align_gmms(gmmfixed::Union{MolGMM,FeatureMolGMM}, gmmmoving::Union{MolGMM,FeatureMolGMM}, cfixed=Inf, cmoving=Inf, nsplits=2; atol=0.1, rtol=0.01, maxblocks=5e8, maxstagnant=Inf, maxevals=Inf)
    res = GaussianMixtureAlignment.tiv_gogma_align(gmmmoving.model, gmmfixed.model, cmoving, cfixed, nsplits, atol=atol, rtol=rtol, maxblocks=maxblocks, maxevals=maxevals, maxstagnant=maxstagnant)
    return res[1], gogma_tform(res[3]), res[4]
end
